...
for i = 1:length(b)
    ...
    for j = 1:numSimulations
        [simAvgAge(:,j), simAvgWait(:,j)] = TDMA_with_slot_end_correction(tFinal, dt, numSources, slotDuration, lambda, mu, slot_scalar);
    end
...
annotationStr = [annotationStr, 'k = ', int2str(slot_scalar)];
annotationStr = {annotationStr, 'T_1 = k * b * T', 'T_2 = k * (1 - b) * T'};
